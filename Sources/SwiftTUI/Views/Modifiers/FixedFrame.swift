...
extension FixedFrame {
 init(content: Content, width: Int? = nil, height: Int? = nil, alignment: Alignment) {
  self.content = content
  if let width { self.width = Extended(width) } else { self.width = nil }
  if let height { self.height = Extended(height) } else { self.height = nil }
  self.alignment = alignment
 }
}
